...
test('case insensitive query params', t => {
  t.plan(4)
  ...
  fastify.get('/foo', (req, reply) => {
    reply.send({ hello: 'world', query: req.query.test })
  })
  ...
  fastify.listen(0, err => {
    ...
    sget({
      method: 'GET',
      url: 'http://localhost:' + fastify.server.address().port + '/foo?TeSt=CaseInsensitiveQuery'
    }, (err, response, body) => {
      t.error(err)
      t.strictEqual(response.statusCode, 200)
      t.deepEqual(JSON.parse(body), {
